...
--<MY_MEMBER_INFO 테이블>에 업체(INTERIOR)로 분류된 계정이 생성되어 있어야 하고(MEMNO와 기타 회원정보를 얻기 위함),
--<MY_MEMBER_INFO 테이블>과 연결된 <MY_INTERIOR 테이블>에도 동시에 등록되어 있어야 하며(INTENO를 얻기 위함),
--<MY_MEMBER_INFO 테이블>의 MEMNO와 <MY_INTERIOR 테이블>의 INTENO가 세션에 담긴 접속자의 MEMNO, INTENO와 대조하기 위해 쿼리 전반에 많이 사용됩니다.

--예를 들면, 홈-시공사례-삭제 시, 시공사례 테이블에 외래키로 등록된 작성자(업체)의 INTENO와 현재 접속한 계정의 INTENO가 같아야지만 삭제가 가능하도록 처리
--또는, 홈-리뷰-수정 시, 리뷰 테이블에 외래키로 등록된 작성자(회원)의 MEMNO가 현재 접속한 계정의 MEMNO와 같아야지만 수정이 가능하도록 처리

--시공사례, 리뷰 데이터 입력 시 꼭 연결된 BIZ_RV_IMG, BIZ_CASES_IMG 테이블에도 최소 1개 이상 이미지 데이터를 입력해 주세요. (가급적 3개 이상 입력을 권합니다.)
--예를 들면, <INTENO가 1번인 업체가 작성한 시공사례넘버BC_NO 3번 게시물의 이미지 정보>가,
--BIZ_CASES_IMG 테이블 내에도 <INTENO=1, BC_NO=3을 포함한 이미지파일명 데이터>로 반드시 입력되어 있어야 합니다.
--이 작업이 맞춰지지 않으면 홈에서 불러올 때 오류가 납니다.





--데이터 양을 많이 넣으면 에러 메시지 발생하니 프로세스 처리가능 수를 늘려줘야 합니다.
--저는 80개 홈 더미데이터를 넣었더니 종종 오류가 났습니다. 더 필요하시면 아래의 500 숫자 부분을 늘려주시면 됩니다.
...
--테이블 삭제 명령어
drop table BIZ_CASES purge;
drop sequence BIZ_CASES_seq;

drop table BIZ_CASES_IMG purge;
drop sequence BIZ_CASES_IMG_seq;

drop table BIZ_ESTI purge;
drop sequence BIZ_ESTI_seq;

drop table BIZ_HOME purge;
drop sequence BIZ_HOME_seq;

drop table BIZ_HOME_BMARK purge;
drop sequence BIZ_HOME_BMARK_seq;

drop table BIZ_MGZ purge;
drop sequence BIZ_MGZ_seq;

drop table BIZ_MGZ_IMG purge;
drop sequence BIZ_MGZ_IMG_seq;

drop table BIZ_PRICE purge;
drop sequence BIZ_PRICE_seq;

drop table BIZ_RV purge;
drop sequence BIZ_RV_seq;

drop table BIZ_RV_IMG purge;
drop sequence BIZ_RV_IMG_seq;

drop table BIZ_RV_LIKE purge;
drop sequence BIZ_RV_LIKE_seq;

drop table BIZ_RV_REPORTED purge;
drop sequence BIZ_RV_REPORTED_seq;

drop table BIZ_SEARCH_TEMP purge;
drop sequence BIZ_SEARCH_TEMP_seq;

drop table MY_MEMBER_INFO purge;
drop sequence MY_MEMBER_INFO_seq;

drop table MY_REPORT purge;
drop sequence MY_REPORT_seq;
...
--주소서치/전문시공영역서치에서 계약건수순으로 정렬 되는지 보기 위해 계약건수를 임의로 조정
update biz_home set bh_cont_cnt = 5 where inteno=1;              
update biz_home set bh_cont_cnt = 3 where inteno=2;
update biz_home set bh_cont_cnt = 2 where inteno=3;
update biz_home set bh_cont_cnt = 4 where inteno=4;
update biz_home set bh_cont_cnt = 1 where inteno=5;
...
--테이블 CREATE 및 기본 더미데이터 INSERT 명령어

--이 테이블은 서치화면 정렬 기능 사용 시 별점 저장 및 리뷰개수 카운트 저장을 위한 TEMP 테이블로, 사전 데이터 입력이 필요없습니다.
CREATE TABLE BIZ_SEARCH_TEMP(
	BST_NO number,
	BST_STARAVG number,
    BST_RVCNT number,
	INTENO number);

create sequence BIZ_SEARCH_TEMP_seq;           

select * from biz_search_temp;

commit;


-- 리뷰_신고저장용 테이블: 리뷰 신고 시 데이터 발생하며 사전 데이터 입력 필요 없습니다.
-- <신고된 리뷰의 블라인드 처리를 풀 때> 제목과 내용을 다시 가져오기 위해 필요함.
CREATE TABLE BIZ_RV_REPORTED(
	BRR_NO number,
	BRR_CONTENT varchar2(1000),
    BRR_WRITER varchar2(200),
    BR_NO number
);

create sequence biz_rv_reported_seq;

select * from biz_rv_reported;

commit;


--MY의 신고 테이블에 신고된 리뷰 정보를 입력하기 위해 임시로 만들어 사용했던 테이블로, MY의 SQL 내에 있을 것임. 사전 데이터 입력 필요 없습니다.
...
--MY의 멤버 테이블에 인테리어 업체 계정을 생성하기 위해 임시로 만들어 사용했던 테이블로, MY의 SQL 내에 있을 것임.
--해당 인테리어 업체만 글 수정/삭제 가능하도록, 업체 정보 수정 가능하도록 만들 때: INTENO가 있는 업체계정이어야만 테스트가 가능해서 더미데이터 생성하여 사용.
--리뷰 수정 삭제 기능 등을 활용할 때는 MEMNO를 대조해야 하기 때문에 PERSON 계정도 필요합니다. (아직 세션 수정 중이라 거기까지 테스트 못하는 중)
...
--
--Insert into IBARA.MY_MEMBER_INFO (MEMNO,NICKNAME,SHPWD,BCPWD,EMAIL,MAILCHECK,BIRTH,GENDER,PHONE,JOINDATE,MEMTYPE,PROFILEIMG) values (5,'호호','c1V5p7bjUgxsam+O7Sd1Mag5AOTE+B/QGQ2sBHwsxcab4iqF4rxC2Pj1kEEQM/mi32JUq+g6skNYIO7KQA5cOg==','qx13bnXk+QS5lRPoLH8+Mg==','amine1225@gmail.com',1,'20231212','notCheck','01012345678',to_timestamp('24/03/25 14:36:44.407000000','RR/MM/DD HH24:MI:SSXFF'),'INTERIOR','1711437904806_car1.png');
--Insert into IBARA.MY_MEMBER_INFO (MEMNO,NICKNAME,SHPWD,BCPWD,EMAIL,MAILCHECK,BIRTH,GENDER,PHONE,JOINDATE,MEMTYPE,PROFILEIMG) values (4,'내가판매왕','c1V5p7bjUgxsam+O7Sd1Mag5AOTE+B/QGQ2sBHwsxcab4iqF4rxC2Pj1kEEQM/mi32JUq+g6skNYIO7KQA5cOg==','qx13bnXk+QS5lRPoLH8+Mg==','abc@naver.com',1,'20231212','notCheck','01012345678',to_timestamp('24/04/05 10:38:22.665000000','RR/MM/DD HH24:MI:SSXFF'),'INTERIOR','1712281241491_heart.png');
--Insert into IBARA.MY_MEMBER_INFO (MEMNO,NICKNAME,SHPWD,BCPWD,EMAIL,MAILCHECK,BIRTH,GENDER,PHONE,JOINDATE,MEMTYPE,PROFILEIMG) values (3,'smsm','c1V5p7bjUgxsam+O7Sd1Mag5AOTE+B/QGQ2sBHwsxcab4iqF4rxC2Pj1kEEQM/mi32JUq+g6skNYIO7KQA5cOg==','qx13bnXk+QS5lRPoLH8+Mg==','smdragon@naver.com',1,'20231212','notCheck','01012345678',to_timestamp('24/03/28 14:15:54.360000000','RR/MM/DD HH24:MI:SSXFF'),'INTERIOR','1712647765373_ia.png');
--Insert into IBARA.MY_MEMBER_INFO (MEMNO,NICKNAME,SHPWD,BCPWD,EMAIL,MAILCHECK,BIRTH,GENDER,PHONE,JOINDATE,MEMTYPE,PROFILEIMG) values (2,'sksksk','c1V5p7bjUgxsam+O7Sd1Mag5AOTE+B/QGQ2sBHwsxcab4iqF4rxC2Pj1kEEQM/mi32JUq+g6skNYIO7KQA5cOg==','qx13bnXk+QS5lRPoLH8+Mg==','sm_dragon@naver.com',1,'20231212','notCheck','0105487542',to_timestamp('24/04/07 14:49:07.264000000','RR/MM/DD HH24:MI:SSXFF'),'INTERIOR','1712537825085_ila.jpg');
--Insert into IBARA.MY_MEMBER_INFO (MEMNO,NICKNAME,SHPWD,BCPWD,EMAIL,MAILCHECK,BIRTH,GENDER,PHONE,JOINDATE,MEMTYPE,PROFILEIMG) values (1,'admin','c1V5p7bjUgxsam+O7Sd1Mag5AOTE+B/QGQ2sBHwsxcab4iqF4rxC2Pj1kEEQM/mi32JUq+g6skNYIO7KQA5cOg==','qx13bnXk+QS5lRPoLH8+Mg==','admin',1,'20240320','notCheck','01012341234',to_timestamp('24/03/27 09:53:52.366000000','RR/MM/DD HH24:MI:SSXFF'),'ADMIN',null);
--
...
--상동. 같은 목적으로 INTENO를 얻기 위해 임시로 생성해 사용한 MY 내의 연결 테이블입니다.
--create table my_interior(
--inteno number,
--memno number,
--interiornum varchar2(12));
--
--create sequence inteno_seq;
--
--Insert into my_interior values(inteno_seq.nextval, 1, 11111111111);
--Insert into my_interior values(inteno_seq.nextval, 2, 11111111111);
--Insert into my_interior values(inteno_seq.nextval, 3, 11111111111);
--Insert into my_interior values(inteno_seq.nextval, 4, 11111111111);
--Insert into my_interior values(inteno_seq.nextval, 5, 11111111111);


--홈 테이블로, 모든 업체에는 홈이 반드시 생성되어 있어야 합니다.
--추후 홈 자동생성 쿼리를 MY 페이지 내 사업자등록번호 승인 버튼에 추가할 예정이나, 시간 부족 시 이미 짝맞춰 생성해둔 홈 데이터로 그냥 시연하겠음.
--더미데이터 입력 시, 주소로 검색 메인페이지의 무한스크롤 및 전문시공검색 내 페이지네이션 기능을 테스트하기 위해서 최소 13개 이상 입력을 권장합니다.(12개까지는 기능 테스트 불가함)
--아마 홈/리뷰/시공사례가 가장 많은 데이터 입력이 필요할 것 같습니다. view단 구성상 홈마다 최소 3개씩의 리뷰/시공사례가 존재하면 좋겠고, 시연 시에는 리얼리티를 위해 3개보다 많은 데이터가 들어간 홈들을 보여줄 수 있으면 좋겠습니다.
...
SELECT * FROM BIZ_HOME;

commit;


--접속한 회원이 마음에 들어서 북마크를 찍은 업체(홈) 정보: 사전 데이터 입력이 필요 없습니다.
--컨트롤러 내에서는 BIZ_HOME 테이블 내 BH_BOOKMARKED 북마크 개수 칼럼과 연동. 생성시 해당홈 북마크 개수 +1, 삭제 시 북마크개수 -1 되는 함수가 있음.
...
--회원들이 업체(홈)에 작성한 리뷰가 모두 모여있는 테이블
--내용이 너무 길어져서 4000을 넘어가면 오류가 나니 4000보다는 짧으면서 몇 줄 정도 길이가 되게 들어가면 좋겠습니다.
--최상단에 기입한 내용대로 반드시 연결된 IMG 데이터가 있어야 합니다.
--별점 평균으로 정렬 기능을 테스트하기 위해 맨뒤의 별점과 업체넘버INTENO, 작성자넘버MEMNO를 바꿔가면서 여러 개 반복 기입해서 사용했습니다.
--(리뷰글 내에 엔터 사용을 하지 않고 테스트 했는데, 엔터를 쓰면 어떻게 될지 잘 모르겠습니다.)
CREATE TABLE BIZ_RV(
	BR_NO number,
	BR_CONTENT varchar2(4000),
    BR_WRITER varchar2(200),
	BR_LIKE_CNT number,
	BR_DATE timestamp,
	BR_POINT number,
	INTENO number,
...
insert into biz_rv values (biz_rv_seq.nextval, '유튜버 라온 저음이 되게 좋당', '라온', 0, systimestamp, 9.8, 1, 2);
insert into biz_rv values (biz_rv_seq.nextval, '파묘 또 보고싶어 김고은조아', '김고은', 0, systimestamp, 9.7, 2, 2);
insert into biz_rv values (biz_rv_seq.nextval, '하반기에 퇴마록도 빨리 나왔으면 좋겠당', '승희', 0, systimestamp, 9.6, 3, 2);
insert into biz_rv values (biz_rv_seq.nextval, '수영장 티켓팅 까먹지 말아야지 이번달엔', '사설가기싫어', 0, systimestamp, 9.4, 4, 2);
insert into biz_rv values (biz_rv_seq.nextval, '로또는 안 사지만 로또 됐음 좋겠다 그럼 놀면서 개발할 텐데', '일확천금', 0, systimestamp, 9.3, 5, 2);
...
--위 테이블과 연결된 이미지 저장용 테이블입니다. 각 리뷰당 반드시 이미지가 들어가 있어야 합니다.
--아직 리뷰 삭제 시 이미지 삭제 기능은 구현하지 않았기 때문에(실제로 시연에 필요하지 않아 시간 보고 구현 안 할지도 모르겠음), 사이트 내에서 리뷰를 삭제하셔도 해당 입력 이미지는 그대로 남아 있습니다.
...
--더미데이터 입력 시에는 파일 3장만 넣어 두고 잘 나오는지만 테스트하며 돌려 썼습니다.
--첫줄이 다 똑같으면 다른 홈의 리뷰이미지 및 데이터가 연결되어도 구분하기 어려워서, 이미지 순서를 바꿔가며 더미데이터를 썼습니다.
insert into biz_rv_img values(biz_rv_img_seq.nextval, 'orifile.png','rv_dummy1.jpg', 1);
insert into biz_rv_img values(biz_rv_img_seq.nextval, 'orifile.png','rv_dummy2.jpg', 1);
insert into biz_rv_img values(biz_rv_img_seq.nextval, 'orifile.png','rv_dummy3.png', 1);

insert into biz_rv_img values(biz_rv_img_seq.nextval, 'orifile.png','rv_dummy3.jpg', 2);
insert into biz_rv_img values(biz_rv_img_seq.nextval, 'orifile.png','rv_dummy2.jpg', 2);
insert into biz_rv_img values(biz_rv_img_seq.nextval, 'orifile.png','rv_dummy1.png', 2);

insert into biz_rv_img values(biz_rv_img_seq.nextval, 'orifile.png','rv_dummy2.jpg', 3);
insert into biz_rv_img values(biz_rv_img_seq.nextval, 'orifile.png','rv_dummy3.jpg', 3);
insert into biz_rv_img values(biz_rv_img_seq.nextval, 'orifile.png','rv_dummy1.png', 3);

insert into biz_rv_img values(biz_rv_img_seq.nextval, 'orifile.png','rv_dummy1.jpg', 4);
insert into biz_rv_img values(biz_rv_img_seq.nextval, 'orifile.png','rv_dummy2.jpg', 4);
insert into biz_rv_img values(biz_rv_img_seq.nextval, 'orifile.png','rv_dummy3.png', 4);

insert into biz_rv_img values(biz_rv_img_seq.nextval, 'orifile.png','rv_dummy3.jpg', 5);
insert into biz_rv_img values(biz_rv_img_seq.nextval, 'orifile.png','rv_dummy2.jpg', 5);
insert into biz_rv_img values(biz_rv_img_seq.nextval, 'orifile.png','rv_dummy1.png', 5);

insert into biz_rv_img values(biz_rv_img_seq.nextval, 'orifile.png','rv_dummy1.jpg', 6);
insert into biz_rv_img values(biz_rv_img_seq.nextval, 'orifile.png','rv_dummy2.jpg', 6);
insert into biz_rv_img values(biz_rv_img_seq.nextval, 'orifile.png','rv_dummy3.png', 6);

insert into biz_rv_img values(biz_rv_img_seq.nextval, 'orifile.png','rv_dummy3.jpg', 7);
insert into biz_rv_img values(biz_rv_img_seq.nextval, 'orifile.png','rv_dummy2.jpg', 7);
insert into biz_rv_img values(biz_rv_img_seq.nextval, 'orifile.png','rv_dummy1.png', 7);

insert into biz_rv_img values(biz_rv_img_seq.nextval, 'orifile.png','rv_dummy2.jpg', 8);
insert into biz_rv_img values(biz_rv_img_seq.nextval, 'orifile.png','rv_dummy3.jpg', 8);
insert into biz_rv_img values(biz_rv_img_seq.nextval, 'orifile.png','rv_dummy1.png', 8);

insert into biz_rv_img values(biz_rv_img_seq.nextval, 'orifile.png','rv_dummy1.jpg', 9);
insert into biz_rv_img values(biz_rv_img_seq.nextval, 'orifile.png','rv_dummy2.jpg', 9);
insert into biz_rv_img values(biz_rv_img_seq.nextval, 'orifile.png','rv_dummy3.png', 9);

insert into biz_rv_img values(biz_rv_img_seq.nextval, 'orifile.png','rv_dummy3.jpg', 10);
insert into biz_rv_img values(biz_rv_img_seq.nextval, 'orifile.png','rv_dummy2.jpg', 10);
insert into biz_rv_img values(biz_rv_img_seq.nextval, 'orifile.png','rv_dummy1.png', 10);

insert into biz_rv_img values(biz_rv_img_seq.nextval, 'orifile.png','rv_dummy1.jpg', 11);
insert into biz_rv_img values(biz_rv_img_seq.nextval, 'orifile.png','rv_dummy2.jpg', 11);
insert into biz_rv_img values(biz_rv_img_seq.nextval, 'orifile.png','rv_dummy3.png', 11);

insert into biz_rv_img values(biz_rv_img_seq.nextval, 'orifile.png','rv_dummy3.jpg', 12);
insert into biz_rv_img values(biz_rv_img_seq.nextval, 'orifile.png','rv_dummy2.jpg', 12);
insert into biz_rv_img values(biz_rv_img_seq.nextval, 'orifile.png','rv_dummy1.png', 12);

insert into biz_rv_img values(biz_rv_img_seq.nextval, 'orifile.png','rv_dummy2.jpg', 13);
insert into biz_rv_img values(biz_rv_img_seq.nextval, 'orifile.png','rv_dummy3.jpg', 13);
insert into biz_rv_img values(biz_rv_img_seq.nextval, 'orifile.png','rv_dummy1.png', 13);

insert into biz_rv_img values(biz_rv_img_seq.nextval, 'orifile.png','rv_dummy1.jpg', 14);
insert into biz_rv_img values(biz_rv_img_seq.nextval, 'orifile.png','rv_dummy2.jpg', 14);
insert into biz_rv_img values(biz_rv_img_seq.nextval, 'orifile.png','rv_dummy3.png', 14);

insert into biz_rv_img values(biz_rv_img_seq.nextval, 'orifile.png','rv_dummy3.jpg', 15);
insert into biz_rv_img values(biz_rv_img_seq.nextval, 'orifile.png','rv_dummy2.jpg', 15);
insert into biz_rv_img values(biz_rv_img_seq.nextval, 'orifile.png','rv_dummy1.png', 15);


commit;


select *from biz_rv_img;


--리뷰에 좋아요 찍기. 사전 데이터 기입이 필요 없습니다.
--추후 좋아요 찍은 리뷰 모아보기 view를 만들 예정. (시간 부족하면 북마크한 홈 모아보기만 만들 것입니다.)
...
select * from biz_rv_like;

commit;


--매거진 테이블. 매거진의 게시물은 제가 서너 개 만들어서 이미지와 함께 채워둘 생각이라 현재 테스트용으로 연결된 더미데이터로 테스트만 해보시면 될 것 같습니다.
--다만, MEMNO는 반드시 admin의 memno여야 합니다. (관리자만 작성/수정/삭제 가능하게 세션 연결 예정)
CREATE TABLE BIZ_MGZ (
	BM_NO number,
	BM_TITLE varchar2(100),
	BM_CONTENT varchar2(3000),
    BM_WRITER varchar2(300),
	BM_DATE timestamp,
	BM_HIT number,
	MEMNO number
);
...
-- 위 게시물들에 첨부된 이미지 게시판. 역시 아래 쿼리로 더미데이터 테스트만 해보시면 될 것 같습니다.
...
select * from biz_mgz_img;


--시공사례 게시판. 리뷰와 함께 다량의 데이터가 들어갈 것 같습니다.
--view 구성 상, 각 업체(홈)마다 최소 3개 이상의 시공사례가 들어가면 좋겠습니다.
--리뷰처럼 반드시 BIZ_CASE_IMG 테이블에 시공사례와 연결된 사진 데이터가 들어가 있어야 합니다.
--내용이 너무 길어져서 4000을 넘어가면 오류가 나니 4000보다는 짧게 작성하여야 하며, 시공사례 게시판인만큼 시연에 사용할 주요 데이터들은 사진이 너무 적지 않았으면 합니다.(오늘의 집 시공사례처럼)
...
insert into biz_cases values(biz_cases_seq.nextval, '1111시공사례입니당1111', '생이란 이 얼마나 허무하고 아름다운가 왜 우린 우리자체로 행복 할수없는가 우린 어디서 와 어디로 가는 중인가 원해 이 모든걸 하나로 아울러주는 답', 'NOAH', systimestamp, 0, 1);
insert into biz_cases values(biz_cases_seq.nextval, '2222시공사례입니당2222', '맨날 재고 재서 난 삐뚤하고 싶어 난 미꾸라지처럼 미끄럽고 싶어 싸우긴 싫어도 입 닥치긴 싫어 그래 난 시끄럽게 쥐뿔 없고 싶어 있으려고 그저 가만있기 싫어 있으려고 가만있기 보단 시끄럽게 쥐뿔 없고 싶어 근데 또 재벌이 되고 싶다고', 'been', systimestamp, 0, 1);
insert into biz_cases values(biz_cases_seq.nextval, '3333시공사례입니당3333', '늘 새롭지 않아도 괜찮아 나에게 너는 언제나 설렘이야 꿈처럼 포근하게 나를 안아준 떨리는 가슴 가득한 봄이니까', 'la boom', systimestamp, 0, 1);

insert into biz_cases values(biz_cases_seq.nextval, '1111시공사례입니당1111', '생이란 이 얼마나 허무하고 아름다운가 왜 우린 우리자체로 행복 할수없는가 우린 어디서 와 어디로 가는 중인가 원해 이 모든걸 하나로 아울러주는 답', 'NOAH', systimestamp, 0, 2);
insert into biz_cases values(biz_cases_seq.nextval, '2222시공사례입니당2222', '맨날 재고 재서 난 삐뚤하고 싶어 난 미꾸라지처럼 미끄럽고 싶어 싸우긴 싫어도 입 닥치긴 싫어 그래 난 시끄럽게 쥐뿔 없고 싶어 있으려고 그저 가만있기 싫어 있으려고 가만있기 보단 시끄럽게 쥐뿔 없고 싶어 근데 또 재벌이 되고 싶다고', 'been', systimestamp, 0, 2);
insert into biz_cases values(biz_cases_seq.nextval, '3333시공사례입니당3333', '늘 새롭지 않아도 괜찮아 나에게 너는 언제나 설렘이야 꿈처럼 포근하게 나를 안아준 떨리는 가슴 가득한 봄이니까', 'la boom', systimestamp, 0, 2);

insert into biz_cases values(biz_cases_seq.nextval, '1111시공사례입니당1111', '생이란 이 얼마나 허무하고 아름다운가 왜 우린 우리자체로 행복 할수없는가 우린 어디서 와 어디로 가는 중인가 원해 이 모든걸 하나로 아울러주는 답', 'NOAH', systimestamp, 0, 3);
insert into biz_cases values(biz_cases_seq.nextval, '2222시공사례입니당2222', '맨날 재고 재서 난 삐뚤하고 싶어 난 미꾸라지처럼 미끄럽고 싶어 싸우긴 싫어도 입 닥치긴 싫어 그래 난 시끄럽게 쥐뿔 없고 싶어 있으려고 그저 가만있기 싫어 있으려고 가만있기 보단 시끄럽게 쥐뿔 없고 싶어 근데 또 재벌이 되고 싶다고', 'been', systimestamp, 0, 3);
insert into biz_cases values(biz_cases_seq.nextval, '3333시공사례입니당3333', '늘 새롭지 않아도 괜찮아 나에게 너는 언제나 설렘이야 꿈처럼 포근하게 나를 안아준 떨리는 가슴 가득한 봄이니까', 'la boom', systimestamp, 0, 3);

insert into biz_cases values(biz_cases_seq.nextval, '1111시공사례입니당1111', '생이란 이 얼마나 허무하고 아름다운가 왜 우린 우리자체로 행복 할수없는가 우린 어디서 와 어디로 가는 중인가 원해 이 모든걸 하나로 아울러주는 답', 'NOAH', systimestamp, 0, 4);
insert into biz_cases values(biz_cases_seq.nextval, '2222시공사례입니당2222', '맨날 재고 재서 난 삐뚤하고 싶어 난 미꾸라지처럼 미끄럽고 싶어 싸우긴 싫어도 입 닥치긴 싫어 그래 난 시끄럽게 쥐뿔 없고 싶어 있으려고 그저 가만있기 싫어 있으려고 가만있기 보단 시끄럽게 쥐뿔 없고 싶어 근데 또 재벌이 되고 싶다고', 'been', systimestamp, 0, 4);
insert into biz_cases values(biz_cases_seq.nextval, '3333시공사례입니당3333', '늘 새롭지 않아도 괜찮아 나에게 너는 언제나 설렘이야 꿈처럼 포근하게 나를 안아준 떨리는 가슴 가득한 봄이니까', 'la boom', systimestamp, 0, 4);

insert into biz_cases values(biz_cases_seq.nextval, '1111시공사례입니당1111', '생이란 이 얼마나 허무하고 아름다운가 왜 우린 우리자체로 행복 할수없는가 우린 어디서 와 어디로 가는 중인가 원해 이 모든걸 하나로 아울러주는 답', 'NOAH', systimestamp, 0, 5);
insert into biz_cases values(biz_cases_seq.nextval, '2222시공사례입니당2222', '맨날 재고 재서 난 삐뚤하고 싶어 난 미꾸라지처럼 미끄럽고 싶어 싸우긴 싫어도 입 닥치긴 싫어 그래 난 시끄럽게 쥐뿔 없고 싶어 있으려고 그저 가만있기 싫어 있으려고 가만있기 보단 시끄럽게 쥐뿔 없고 싶어 근데 또 재벌이 되고 싶다고', 'been', systimestamp, 0, 5);
insert into biz_cases values(biz_cases_seq.nextval, '3333시공사례입니당3333', '늘 새롭지 않아도 괜찮아 나에게 너는 언제나 설렘이야 꿈처럼 포근하게 나를 안아준 떨리는 가슴 가득한 봄이니까', 'la boom', systimestamp, 0, 5);

commit;

select * from biz_cases;


--위와 연결된 이미지 테이블. 리뷰 이미지 테이블과 주의점이 같습니다. 위에 쓰는 걸 깜빡했는데 파일명 길이 제한이 있으니 짧은 파일명을 사용해 주세요.
--더미데이터 사용할 때 이미지가 깨져 나오면 .png, .jpg 형식이 섞인 것이니 같은 형식 파일 하나 동일한 이름으로 upload-biz폴더에 넣어주시면 잘 나옵니다.
...
select * from biz_cases_img;

insert into biz_cases_img values(biz_cases_img_seq.nextval, 'orifile.png','cases_dummy1.jpg', 1, 1);
insert into biz_cases_img values(biz_cases_img_seq.nextval, 'orifile.png','cases_dummy2.jpg', 1, 1);
insert into biz_cases_img values(biz_cases_img_seq.nextval, 'orifile.png','cases_dummy3.png', 1, 1);

insert into biz_cases_img values(biz_cases_img_seq.nextval, 'orifile.png','cases_dummy3.png', 2, 1);
insert into biz_cases_img values(biz_cases_img_seq.nextval, 'orifile.png','cases_dummy2.jpg', 2, 1);
insert into biz_cases_img values(biz_cases_img_seq.nextval, 'orifile.png','cases_dummy1.jpg', 2, 1);

insert into biz_cases_img values(biz_cases_img_seq.nextval, 'orifile.png','cases_dummy3.png', 3, 1);
insert into biz_cases_img values(biz_cases_img_seq.nextval, 'orifile.png','cases_dummy2.jpg', 3, 1);
insert into biz_cases_img values(biz_cases_img_seq.nextval, 'orifile.png','cases_dummy1.jpg', 3, 1);


insert into biz_cases_img values(biz_cases_img_seq.nextval, 'orifile.png','cases_dummy1.jpg', 4, 2);
insert into biz_cases_img values(biz_cases_img_seq.nextval, 'orifile.png','cases_dummy2.jpg', 4, 2);
insert into biz_cases_img values(biz_cases_img_seq.nextval, 'orifile.png','cases_dummy3.png', 4, 2);

insert into biz_cases_img values(biz_cases_img_seq.nextval, 'orifile.png','cases_dummy3.png', 5, 2);
insert into biz_cases_img values(biz_cases_img_seq.nextval, 'orifile.png','cases_dummy2.jpg', 5, 2);
insert into biz_cases_img values(biz_cases_img_seq.nextval, 'orifile.png','cases_dummy1.jpg', 5, 2);

insert into biz_cases_img values(biz_cases_img_seq.nextval, 'orifile.png','cases_dummy3.png', 6, 2);
insert into biz_cases_img values(biz_cases_img_seq.nextval, 'orifile.png','cases_dummy2.jpg', 6, 2);
insert into biz_cases_img values(biz_cases_img_seq.nextval, 'orifile.png','cases_dummy1.jpg', 6, 2);


insert into biz_cases_img values(biz_cases_img_seq.nextval, 'orifile.png','cases_dummy1.jpg', 7, 3);
insert into biz_cases_img values(biz_cases_img_seq.nextval, 'orifile.png','cases_dummy2.jpg', 7, 3);
insert into biz_cases_img values(biz_cases_img_seq.nextval, 'orifile.png','cases_dummy3.png', 7, 3);

insert into biz_cases_img values(biz_cases_img_seq.nextval, 'orifile.png','cases_dummy3.png', 8, 3);
insert into biz_cases_img values(biz_cases_img_seq.nextval, 'orifile.png','cases_dummy2.jpg', 8, 3);
insert into biz_cases_img values(biz_cases_img_seq.nextval, 'orifile.png','cases_dummy1.jpg', 8, 3);

insert into biz_cases_img values(biz_cases_img_seq.nextval, 'orifile.png','cases_dummy3.png', 9, 3);
insert into biz_cases_img values(biz_cases_img_seq.nextval, 'orifile.png','cases_dummy2.jpg', 9, 3);
insert into biz_cases_img values(biz_cases_img_seq.nextval, 'orifile.png','cases_dummy1.jpg', 9, 3);


insert into biz_cases_img values(biz_cases_img_seq.nextval, 'orifile.png','cases_dummy1.jpg', 10, 4);
insert into biz_cases_img values(biz_cases_img_seq.nextval, 'orifile.png','cases_dummy2.jpg', 10, 4);
insert into biz_cases_img values(biz_cases_img_seq.nextval, 'orifile.png','cases_dummy3.png', 10, 4);

insert into biz_cases_img values(biz_cases_img_seq.nextval, 'orifile.png','cases_dummy3.png', 11, 4);
insert into biz_cases_img values(biz_cases_img_seq.nextval, 'orifile.png','cases_dummy2.jpg', 11, 4);
insert into biz_cases_img values(biz_cases_img_seq.nextval, 'orifile.png','cases_dummy1.jpg', 11, 4);

insert into biz_cases_img values(biz_cases_img_seq.nextval, 'orifile.png','cases_dummy3.png', 12, 4);
insert into biz_cases_img values(biz_cases_img_seq.nextval, 'orifile.png','cases_dummy2.jpg', 12, 4);
insert into biz_cases_img values(biz_cases_img_seq.nextval, 'orifile.png','cases_dummy1.jpg', 12, 4);


insert into biz_cases_img values(biz_cases_img_seq.nextval, 'orifile.png','cases_dummy1.jpg', 13, 5);
insert into biz_cases_img values(biz_cases_img_seq.nextval, 'orifile.png','cases_dummy2.jpg', 13, 5);
insert into biz_cases_img values(biz_cases_img_seq.nextval, 'orifile.png','cases_dummy3.png', 13, 5);

insert into biz_cases_img values(biz_cases_img_seq.nextval, 'orifile.png','cases_dummy3.png', 14, 5);
insert into biz_cases_img values(biz_cases_img_seq.nextval, 'orifile.png','cases_dummy2.jpg', 14, 5);
insert into biz_cases_img values(biz_cases_img_seq.nextval, 'orifile.png','cases_dummy1.jpg', 14, 5);

insert into biz_cases_img values(biz_cases_img_seq.nextval, 'orifile.png','cases_dummy3.png', 15, 5);
insert into biz_cases_img values(biz_cases_img_seq.nextval, 'orifile.png','cases_dummy2.jpg', 15, 5);
insert into biz_cases_img values(biz_cases_img_seq.nextval, 'orifile.png','cases_dummy1.jpg', 15, 5);
...
--견적용 자재 가격 게시판
--20개 홈 기준으로 실제 사용가능한 데이터가 완성되어 있습니다.
--시간이 남으면 데이터 활용해서 업체 견적 연결 예정이며, 이하로는 기능이 실제로 쓰이기 전까지는 필요가 없습니다.
CREATE TABLE BIZ_PRICE (
	BP_NO number,
	BP_NAME varchar2(300),
...
insert into biz_price values(biz_price_seq.nextval, '[동화자연마루]마루/나투스진 그란데/사하라라이트', '우드앤화이트 인테리어에 어울리는 밝은 라이트 컬러 마루입니다.', 119000, '마루', '프리미엄', '1.png', 40);
insert into biz_price values(biz_price_seq.nextval, '[동화자연마루]마루/나투스진 그란데/이모션 블랑', '스크래치가 나지 않고 찍힘이 강한 따뜻한 색감의 광폭마루입니다.', 99000, '마루', '스탠다드', '2.png', 40);
insert into biz_price values(biz_price_seq.nextval, '[LX]벽지/지아 패브릭/페인팅 화이트/T1059-01', '따뜻한 톤과 차가운 톤의 인테리어에 모두 어울리는 페인팅 느낌의 벽지입니다.', 54000, '도배·페인트', '프리미엄', '41.png', 40);
insert into biz_price values(biz_price_seq.nextval, '[개나리벽지]벽지/아트북/화이트/57160-1', '차갑지 않은 크림톤의 페인트 느낌이 아는 벽지입니다.', 36000, '도배·페인트', '스탠다드', '42.png', 40);
insert into biz_price values(biz_price_seq.nextval, '[LX]장판/엑스컴포트/텐더 그레이', '두텁고 폭신한 질감의 복원력이 우수한 타일 프린팅 그레이색상 장판입니다.', 127000, '장판·타일', '프리미엄', '81.png', 40);
insert into biz_price values(biz_price_seq.nextval, '[LX]장판/지아자연애/소프트 콘크리트', '부드러운 질감의 눌림 현상 적은 소프트 콘크리트 프린팅 장판입니다.', 51000, '장판·타일', '스탠다드', '82.png', 40);
insert into biz_price values(biz_price_seq.nextval, '[아메리칸스탠다드]양변기/C319500A/C437500T 웨이브 S', '화이트컬러의 스퀘어 웨이브 형태 도기 양변기입니다.', 362000, '욕실', '프리미엄', '121.png', 40);
insert into biz_price values(biz_price_seq.nextval, '[아메리칸스탠다드]양변기/C319500A/C427500T 웨이브 R', '화이트컬러의 라운드 웨이브 형태 도기 양변기입니다.', 288000, '욕실', '스탠다드', '122.png', 40);
insert into biz_price values(biz_price_seq.nextval, '슈티에싱크8027 원홀 주방 수전 (색상 4종)', '오랫동안 깔끔하게 사용 가능한 무광 니켈 소재의 원홀 주방수전입니다.', 290000, '주방', '프리미엄', '161.png', 40);
insert into biz_price values(biz_price_seq.nextval, '슈티에싱크1257 수입 투홀 주방수전 (색상 3종)', '모던한 디자인의 투홀형 수입산 버튼식 주방수전입니다.', 226000, '주방', '스탠다드', '162.png', 40);
insert into biz_price values(biz_price_seq.nextval, '엘리세 양개 아치 중문 시공 (기본 강화유리)', '아치형 곡선 타공디자인으로 공간을 한층 부드럽게 만들어주는 다양한 컬러의 아치 중문입니다.', 1081000, '문·샷시', '프리미엄', '201.png', 40);
insert into biz_price values(biz_price_seq.nextval, '프렌치 양개도어 중문 시공', '노출형 경칩이 아닌 상하 잡아주는 매립형 베어링힌지를 사용해 소음 발생을 최소화한 빈틈없는 라운드바 구조의 양면개방식 중문입니다.', 679000, '문·샷시', '스탠다드', '202.png', 40);
insert into biz_price values(biz_price_seq.nextval, 'LED COB 실린더 매입기구 10W_2colors', '심플한 디자인과 자유로운 회전이 가능한 매입형 조명입니다.', 70000, '조명', '프리미엄', '241.png', 40);
insert into biz_price values(biz_price_seq.nextval, '투투 업라이트 LED 반매입등 10W 조명_2colors_2type', '새로운 빛의 형태, 빛이 아래에서 위를 향하는 간접등 스타일로 제작되어 무드가 느껴지는 부드러운 공간을 만들어주는 매입등입니다.', 14000, '조명', '스탠다드', '242.png', 40);
insert into biz_price values(biz_price_seq.nextval, '[레놀릿]필름/S-490364/샌드링햄 오크 그레이', '원목 패턴을 재현한 시트지로 비치지 않으며 고품질의 인테리어필름입니다.', 24000, '시트·필름', '프리미엄', '281.png', 40);
insert into biz_price values(biz_price_seq.nextval, '[영림]필름/PS010/중백색', '영림 스테디셀러 인테리어 필름으로 안정적이고 자연스러운 재질이며 완전한 화이트톤 연출이 가능합니다.', 12000, '시트·필름', '스탠다드', '282.png', 40);