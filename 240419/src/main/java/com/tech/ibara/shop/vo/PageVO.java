...
public class PageVO {
	...
	// 페이지 - Page
	private Integer groupPageCount = 3;   // 페이지 그룹당 페이지 수
	...
	public void pageAndPostCalculate(Integer postTotalNum) {
		// *** 페이지 - Page ***
		// 현재 페이지 번호 - null or 0일 경우 처리
		getPageSelectedNum();
		// 전체 게시글 수
		...
		pageTotalNum = (int) (postTotalNum / outputPostCount);
		// if 조건문 - (전체 게시글 수 / 출력할 게시글 수)의 나머지 값 > 0
		if(postTotalNum % outputPostCount > 0) {
			// 전체 페이지 수, 1 증가
			pageTotalNum++;
		}
